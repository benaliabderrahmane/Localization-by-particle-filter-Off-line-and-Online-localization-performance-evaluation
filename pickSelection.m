...
AllOptions.NR = [16 32 64]; %number of rays
...
for m=1:1
    for i=1:14
        for j=6:6%length(AllOptions.NParticles)
            for jjj=1:2%length(AllOptions.Selection)
                % for each StudyCase
                here = tic;
                Options.Likelihood = AllOptions.Likelihood(1);
                Options.Selection = AllOptions.Selection(jjj);
                ...
                str = strcat(Options.Likelihood," ",Options.Selection," ",Options.Distribution," ",num2str(Options.NParticles)," ",Options.SensorsType," ",num2str(Options.NPP)," ",num2str(Options.MaxSpeed)," ",num2str(Options.NR), " trajectory number ",num2str(i)," test number ",num2str(m), " pick selection")
                ...
                if isfile(filename)
                    ...
                    load(filename);
                    %break 
                    result(i,jjj) = length(Data.iteration);
                else
                    %lunch tests
