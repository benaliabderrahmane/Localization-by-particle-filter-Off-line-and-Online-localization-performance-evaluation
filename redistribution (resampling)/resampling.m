function [Particles,OldParticles,OldRobot,vecteurTconvergence,PPoids,Indice2_] = resampling(Particles,Choice,Obstacles,OldRobot,OldParticles,Nmax,TimeDebut,iteration,vecteurTconvergence,SdX,SdY,SdTheta,SdwX,SdwY,SdwTheta,Poids,Indice2_)
...
%       N: number of particles (in case we change the number of particles when we redistribute)
    ...
    if FlagRedistribution==0 %si on est dans la premiere iteration de convergence
        ...
        case "STANDARD DEVIATION"
            if (SdX<0.2 && SdY<0.2 && SdTheta<0.1)
                N=Nmax;
                particles1=Particles_generator(26.5747,29.02,-0.269984,55,-pi,pi,floor(N/2),Obstacles);
            ...
            case "WHEIGHTED STANDARD DEVIATION"
                max(PPoids)
                if ((SdwX<0.3 && SdwY<0.3) )
                    N=Nmax;
                    particles1=Particles_generator(26.5747,29.02,-0.269984,55,-pi,pi,floor(N/2),Obstacles);
                    particles2=Particles_generator(-5,26.5747,-0.269984,11.53,-pi,pi,N-floor(N/2),Obstacles);
                    particles=[particles1,particles2];
