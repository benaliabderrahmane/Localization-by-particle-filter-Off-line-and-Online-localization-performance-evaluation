...
angles=zeros(1,Mesurement_segmentation);
for i=1:Mesurement_segmentation
    angles(i)=min(Et)+i*astep;
end
...
%Init errors perfs
Mesures=zeros(length(PosSensors(1,:)),3);
Pmax=4;
for i=1:length(PosSensors(1,:))
    [DIST,ximp,yimp,~]=USPATCH_act(Pmax,angles,Obstacles,tmp(1,i),tmp(2,i),PosSensors(3,i),0,CoefTexture_Wall);
    %[DIST,ximp,yimp,GrandObstacle]=USPATCH(Portee,angles,Obstacles,x,y,theta,Bruitage,CoefTexture_Wall)
    [D,I]=min(DIST);
    Mesures(i,:)=[ximp(I) yimp(I) D];
end
